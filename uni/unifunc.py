...
class Uniform:
    ...
    def GAM_R_F(self):
        ...
        return ENV['DELTA_R_F'] / (epsilon
                                   + 1j * np.sqrt(ENV['DELTA_R_F']
                                                  + np.conj(ENV['DELTA_R_F'])
                                                  - epsilon * epsilon))
    ...
    def GAM_R_B(self):
        ...
        return -np.conj(ENV['DELTA_R_B']) / (epsilon + 1j *
                                             np.sqrt(ENV['DELTA_R_B']
                                                     * np.conj(ENV['DELTA_R_B'])
                                                     - epsilon * epsilon))
    ...
    def GAM_A_F(self):
        ...
        return ENV['DELTA_A_F'] / (epsilon + 1j *
                                   np.sqrt(ENV['DELTA_A_F']
                                           * np.conj(ENV['DELTA_A_F'])
                                           - epsilon * epsilon))
    ...
    def GAM_A_B(self):
        ...
        return -np.conj(ENV['DELTA_A_B']) / (epsilon + 1j *
                                             np.sqrt(ENV['DELTA_A_B']
                                                     * np.conj(ENV['DELTA_A_B'])
                                                     - epsilon * epsilon))
